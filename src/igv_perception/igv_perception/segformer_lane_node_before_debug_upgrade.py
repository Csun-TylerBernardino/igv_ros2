...
def read_xyz_at_uv(cloud_msg, x_offset, y_offset, z_offset, u, v):
    if u < 0 or v < 0 or u >= int(cloud_msg.width) or v >= int(cloud_msg.height):
        return None

    point_index = v * int(cloud_msg.width) + u
    base = point_index * int(cloud_msg.point_step)

    fmt = '>' if cloud_msg.is_bigendian else '<'
    data = cloud_msg.data

    x = struct.unpack_from(fmt + 'f', data, base + x_offset)[0]
    y = struct.unpack_from(fmt + 'f', data, base + y_offset)[0]
    z = struct.unpack_from(fmt + 'f', data, base + z_offset)[0]

    if not np.isfinite(x) or not np.isfinite(y) or not np.isfinite(z):
        return None

    dist = float(np.sqrt(x * x + y * y + z * z))
    if dist > TRUE_CLOUD_MAX_RANGE_M:
        return None
...
class SegformerLaneNode(Node):
    ...
    def cloud_callback(self, msg):
        ...
        try:
            self.latest_xyz_offsets = find_xyz_offsets(msg)
        except Exception as e:
            self.latest_xyz_offsets = None
            self.get_logger().error(f'PointCloud field parse error: {e}')

    def publish_lane_cloud(self, mask):
        if self.latest_cloud is None or self.latest_xyz_offsets is None:
            return

        image_h, image_w = mask.shape[:2]
        cloud_w = int(self.latest_cloud.width)
        cloud_h = int(self.latest_cloud.height)

        if cloud_w <= 0 or cloud_h <= 0:
            return
        ...
        if len(xs) == 0:
            ...
            empty_cloud = point_cloud2.create_cloud_xyz32(self.latest_cloud.header, [])
            self.lane_cloud_pub.publish(empty_cloud)
            return

        xs = xs[::TRUE_CLOUD_SAMPLE_STEP]
        ys = ys[::TRUE_CLOUD_SAMPLE_STEP]

        if image_w > 1:
            u = np.clip(
                np.round(xs.astype(np.float32) * (cloud_w - 1) / (image_w - 1)).astype(np.int32),
                0,
                cloud_w - 1
            )
        else:
            u = np.zeros_like(xs, dtype=np.int32)

        if image_h > 1:
            v = np.clip(
                np.round(ys.astype(np.float32) * (cloud_h - 1) / (image_h - 1)).astype(np.int32),
                0,
                cloud_h - 1
            )
        else:
            v = np.zeros_like(ys, dtype=np.int32)

        x_offset, y_offset, z_offset = self.latest_xyz_offsets
        points = []

        try:
            for px, py in zip(u.tolist(), v.tolist()):
                xyz = read_xyz_at_uv(
                    self.latest_cloud,
                    x_offset,
                    y_offset,
            ...
            self.get_logger().error(f'True lane cloud extraction error: {e}')
            return
